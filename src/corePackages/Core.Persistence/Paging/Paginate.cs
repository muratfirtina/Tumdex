...
/*public class CustomListToPaginateConverter<TSource, TDestination> : ITypeConverter<List<TSource>, IPaginate<TDestination>>
{
    public IPaginate<TDestination> Convert(List<TSource> source, IPaginate<TDestination> destination, ResolutionContext context)
    {
        var mappedItems = context.Mapper.Map<List<TDestination>>(source);
        // Burada IPaginate<T> tipinin bir örneğini oluşturup döndürün.
        // Örneğin, bir Paginate<T> nesnesi oluşturabilir ve gerekli sayfalama bilgilerini ayarlayabilirsiniz.
        return new Paginate<TDestination>(mappedItems, 0, 10, mappedItems.Count);
    }
}*/
