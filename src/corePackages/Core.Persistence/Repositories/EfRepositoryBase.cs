    ...
    where TContext : DbContext
{
    ...
    )
    {
        ...
        if (orderBy != null)
            ...
            return await queryable.OrderByDescending(x => x.CreatedDate)
                .ToPaginateAsync(index, size, from: 0, cancellationToken);
    }
    ...
    )
    {
        ...
        // Dynamic sorguda sıralama yoksa varsayılan sıralama ekle
        if (dynamic.Sort?.Any() != true)
            queryable = queryable.OrderByDescending(x => x.CreatedDate);

        return await queryable.ToPaginateAsync(index, size, from: 0, cancellationToken);
    }

    public async Task<List<TEntity>> GetAllByDynamicAsync(
        DynamicQuery dynamic,
        Expression<Func<TEntity, bool>>? predicate = null,
        Func<IQueryable<TEntity>, IIncludableQueryable<TEntity, object>>? include = null,
        int index = -1,
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        // Eğer sayfalama parametreleri verilmişse uygula
        if (index != -1 && size != -1)
            queryable = queryable.Skip(index * size).Take(size);
    ...
    )
    {
        ...
        return queryable.OrderByDescending(x => x.CreatedDate).ToPaginate(index, size);
    }
